...
def gauge_chart(pct, risk):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "threshold": {"line": {"color": color, "width": 3}, "thickness": 0.78, "value": pct},
        },
        title={
            "text": f"<b>{risk}</b>",
...
def donut_chart(compliant, partial, non, na):
    ...
    fig = go.Figure(go.Pie(
        ...
        hole=0.62,
        marker=dict(colors=[COLORS[l] for l in labels], line=dict(color=PIE_LINE, width=2)),
        textinfo="percent",
...
def bar_by_file(all_data):
    ...
    rows = []
    for df in all_data:
        name = df["_source"].iloc[0].replace(".xlsx", "").replace("_", " ")
...
def section_chart(combined):
    """
    Grouped bar chart with a compliance-% line overlay per section.
    Far more informative than raw counts alone.
    """
    CHART_FONT, GRID_COLOR, GAUGE_BG, PIE_LINE = _chart_settings()
    ...
    bar_cols = [c for c in ["Compliant", "Partially Compliant", "Not Compliant"] if c in pivot.columns]
...
def show_module3():
    ...
    if not uploaded_files:
        ...
        <div class="rbi-card" style="text-align:center; padding:52px 32px; color:var(--text-dim);">
            ...
            <div style="font-size:12px; color:var(--text-dim); margin-top:8px;">
                Accepts .xlsx — upload one or multiple annexes simultaneously
            </div>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    if not all_data:
        st.warning("No valid data found. Ensure files have a 'Control Implemented?' column.")
        return
    ...
    with g2:
        st.markdown('<div class="rbi-label" style="margin-bottom:4px; margin-left:6px;">STATUS SPLIT</div>', unsafe_allow_html=True)
        st.plotly_chart(donut_chart(compliant, partial, non, na), use_container_width=True)

    with g3:
        st.markdown('<div class="rbi-label" style="margin-bottom:4px; margin-left:6px;">BY FILE / ANNEX</div>', unsafe_allow_html=True)
    ...
    if fig_sec:
        ...
        st.markdown('<div class="rbi-section-title">Section-wise Breakdown  <span style="font-size:10px; color:var(--text-dim); font-weight:400;">— bars = control counts · gold line = compliance %</span></div>', unsafe_allow_html=True)
